      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold">{user?.full_name || 'מעבדה'}</h2>
          ...
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative h-10 w-10 rounded-full">
                  <Avatar className="h-10 w-10">
                    ...
                    <AvatarFallback>
                      {user?.full_name?.split(' ').map((n: string) => n[0]).join('') || 'L'}
                    </AvatarFallback>
                  </Avatar>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => router.push('/lab/profile')} className="gap-2 justify-end">
                  <User className="ms-2 h-4 w-4" />
                  <span>פרופיל</span>
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push('/lab/settings')} className="gap-2 justify-end">
